...
Status: Foundation implemented through deliverable 10; CI, dependency automation, GitHub controls, and backup operations remain open
...
- There is no CI workflow, dependency automation, GitHub collaboration baseline, backup and restore procedure, release artifact, or deployment.
...
- Backup and restore have not been implemented or verified.
...
Deliverables 1 through 10 are implemented and verified. The following remain, in this order:

1. Deliverable 11, path-filtered CI, calling the same `scripts/verify.sh` commands rather than reimplementing them. CI must install `shellcheck`, which was unavailable locally.
2. Deliverable 12, dependency automation with weekly grouped updates and no auto-merge.
3. Deliverable 13, the GitHub collaboration baseline: labels, issue forms, `SECURITY.md`, least-privilege Actions permissions, and a `main` ruleset once required checks exist.
4. Deliverable 14, documented and proven PostgreSQL backup and restore using non-sensitive test data.

Items 1 through 3 change repository settings and external configuration. They require explicit authorization from the owner before execution.

Outstanding verification gaps:

- The running interface has not been inspected in a real browser. Localization, direction, and theme behavior are covered by automated render tests only. Browser preview automation was unavailable during implementation.
- `scripts/try-pr.sh` has not been exercised end to end against a real pull request, because none exists yet.
- `shellcheck` static analysis was skipped locally and must run in CI.
