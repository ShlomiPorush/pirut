...
- The loopback-only security boundary is verified on every detached start: `scripts/local.sh` inspects the running container and stops the web service if any published port is not bound to `127.0.0.1`. This was added after the machine-local development Compose file was found publishing `4610:4610`, which made the application answer on the Windows LAN address.
